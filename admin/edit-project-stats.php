...
##########Update Query################
	...
	if(isset($_POST['banner_update']))
	{
       		...
       		$query="UPDATE `$page` SET `id`='".$did."' ,`name`='".$name."',`number`='".$number."' WHERE `id`='".$did."'";
			mysqli_query($dbcon,$query) or die(mysqli_error());
			$empty="File Updated Successfully";
			echo'<script>
			alert("Updated Successfully");
			window.location.href="add-projects-stats.php";
			</script>';
                  ...
                  <div class="x_content">
                    <form id="demo-form2" class="form-horizontal form-label-left" method="post" enctype="multipart/form-data">
                       <div class="form-group">
						...
						<div class="form-group"  >
							 <label for="discription">Edit Title</label> 
							 <input type="text" name="name" class="form-control" value="<?php echo $res3['name'];  ?>">
							
						</div>
              <div class="form-group"  >
               <label for="discription">Edit Stats</label> 
               <input type="text" name="number" class="form-control" value="<?php echo $res3['number'];  ?>">
